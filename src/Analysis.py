...
def Airdna(cityId='', regionId=''):
    ...
    params = (
        ('access_token', 'OTgyNTM1|58b6b9cceb7446618890ac0ed048125e'),
        search,
        ('start_month', '10'),
        ('start_year', '2018'),
        ('number_of_months', '36'),
...
counter = {0: 0,
           ...
           5: 0,
           6: 0}
AirdnaRes, areaName = Airdna(cityId= 77100) # Set the city or region ID here
workbook = load_workbook(filename="../Template.xlsx")
sheet = workbook['Template']
sheet.title = areaName + " PY"
min_days = 300
min_occ_allTheRooms = .50
min_reviews = 10
for listing in AirdnaRes:
    ...
    if listing["days_available"] < min_days or listing['room_type'] \
            == 'Private room' or listing['room_type'] == 'Shared room"' or allTheRoomsRes is None or listing['reviews'] <= min_reviews or allTheRoomsRes['occupancyRate'] is None or allTheRoomsRes['occupancyRate'] < min_occ_allTheRooms:
        continue
    ...
    if listing["bedrooms"] == 0:
        ...
        sheet.cell(row=26, column=4 + count).value = "Listing " + str(count)
        sheet.cell(row=26, column=4 + count).comment = Comment(
            "AllTheRooms:" + json.dumps(allTheRoomsRes, indent=4) + "\n\nAirDna:" + json.dumps(listing, indent=4), 'PY',
            948, 260)
        sheet.cell(row=26, column=4 + count).hyperlink = "https://www.airbnb.com/rooms/" + str(
            listing["airbnb_property_id"])
        sheet.cell(row=27, column=4 + count).value = allTheRoomsRes["occupancyRate"]
        sheet.cell(row=28, column=4 + count).value = listing["adr"]

    elif listing["bedrooms"] == 5:
        sheet.cell(row=32, column=4 + count).value = "Listing " + str(count)
        sheet.cell(row=32, column=4 + count).comment = Comment(
            "AllTheRooms:" + json.dumps(allTheRoomsRes, indent=4) + "\n\nAirDna:" + json.dumps(listing, indent=4), 'PY',
            948, 260)
        sheet.cell(row=32, column=4 + count).hyperlink = "https://www.airbnb.com/rooms/" + str(
            listing["airbnb_property_id"])
        sheet.cell(row=33, column=4 + count).value = allTheRoomsRes["occupancyRate"]
        sheet.cell(row=34, column=4 + count).value = listing["adr"]

    elif listing["bedrooms"] >= 5:
        sheet.cell(row=38, column=4 + count).value = "Listing " + str(count)
        sheet.cell(row=38, column=4 + count).comment = Comment(
            "AllTheRooms:" + json.dumps(allTheRoomsRes, indent=4) + "\n\nAirDna:" + json.dumps(listing, indent=4), 'PY',
            948, 260)
        sheet.cell(row=38, column=4 + count).hyperlink = "https://www.airbnb.com/rooms/" + str(
            listing["airbnb_property_id"])
        sheet.cell(row=39, column=4 + count).value = allTheRoomsRes["occupancyRate"]
        sheet.cell(row=40, column=4 + count).value = listing["adr"]
    else:
        print("Bedroom>4 ----> " + json.dumps(listing, indent=4))

workbook.save('../'+areaName+"_"+str(min_days)+'days.xlsx')
